...
- **`subagent_activity`** stays **on** (Telegram) — when the agent delegates to subagents, one edit-in-place "🔀 Subagents · 1/3 done" bubble tracks each child's phase and tool count, including detached children that finish after the parent turn. It shows structure only — never a child's goal, arguments or output. This is best-effort, last-observed progress: a gateway restart or a child that exits without a completion event can leave the displayed phase stale.
