...
- **`subagent_activity_board`** stays **on** (Telegram) — this per-turn observability surface uses one edit-in-place "🔀 Subagents · 1/3 done" bubble to track each child's phase and tool count, including detached children that finish after the parent turn. It shows structure only — never a child's goal, arguments or output. This is best-effort, last-observed progress: a gateway restart or a child that exits without a completion event can leave the displayed phase stale.
